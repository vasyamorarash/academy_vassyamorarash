...
if (isset ($_POST['send'])){
		...
		if(($nameTrophy!="") && ($year!="")){
			...
			if ((strlen($nameTrophy) >= 1) && (strlen($nameTrophy) <= 50)){
   			...
   			if($flag){   				
   				$sql = "INSERT INTO `trophys` (`name`, `year`) 
   							VALUES ('$nameTrophy','$year')";				
...
<div class="delete">
	...
	<?php
		if (isset ($_POST['delete'])){
			...
			$sql = "DELETE FROM `clubs_trophys` WHERE `clubs_trophys`.`trophy_id` = $trophyId";
			$result_id = mysql_query($sql);
			$sql = "DELETE FROM `trophys` WHERE `trophys`.`id` = $trophyId";
			$result_id = mysql_query($sql);
		}
	?>
</div>